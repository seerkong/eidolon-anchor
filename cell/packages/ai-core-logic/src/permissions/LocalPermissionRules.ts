...
export function parseLocalPermissionsConfig(raw: Record<string, unknown>, filePath: string, cwd: string): LocalPermissionsConfig {
  ...
  const overrides: LocalDirectoryOverride[] = overridesRaw.map((value, index) => {
    ...
    return {
      directory: path.resolve(cwd, directoryRaw),
...
export function parseWorkspaceAccessConfig(raw: Record<string, unknown>, filePath: string, cwd: string): WorkspaceAccessConfig {
  assertAbsoluteCwd(cwd);
  ...
  for (const [workspaceKey, workspaceValue] of Object.entries(workspacesObject)) {
    ...
    workspaces[path.resolve(cwd, String(workspaceKey))] = entriesRaw.map((entry, index) => {
      ...
      return {
        path: path.resolve(cwd, entryPathRaw),
        permissions,
      };
    });
  }
...
function assertAbsoluteCwd(cwd: string): void {
  if (!path.isAbsolute(cwd)) {
    throw new LocalPermissionConfigError("Permission parsing requires an absolute cwd");
  }
}
