...
export {
  buildActorTranscriptDirName,
  getActorTranscriptPaths,
  type ActorTranscriptDescriptor,
  type ActorTranscriptPaths,
...
export function serializeMessagesToTranscript(messages: ChatMessage[]): string {
  const records = messagesToTranscriptRecords(messages)
  if (!records.length) return ""
  return StreamTranscript.serialize(records, {
    delimiter: "----",
    includeHeader: true,
    ensureMarker: true,
  })
