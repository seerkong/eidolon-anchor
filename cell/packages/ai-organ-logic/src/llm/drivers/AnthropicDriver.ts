...
import { AnthropicNodejsFetchLlmAdapter } from "../AnthropicNodejsFetchAdapter";
...
export function buildAnthropicProviderDriver(): ProviderDriverDefinition {
  return {
    name: "anthropic",
    adapterNames: ["anthropic", "anthropic-chat", "anthropic_chat"],
    ...
    async createStream(params: ProviderDriverStreamParams) {
      const adapter = new AnthropicNodejsFetchLlmAdapter({
