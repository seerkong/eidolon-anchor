...
import type {
  ...
  EmitFiberSignalParams,
  FiberContext,
  RunStep,
  WaiterStoreResultMap,
...
function publishSchedulerSignal(runtime: AiAgentOrchestratorRuntime, vm: AiAgentVm, updatedAt: number): void {
  ...
  privateRxData.scheduler.set(
    projectSchedulerSignal(runtime.state, updatedAt, Array.from(runtime.pendingResumes)),
  );
}

function applyResumeFiberWithSignal(runtime: AiAgentOrchestratorRuntime, fiberId: string, now: number): void {
  applyResumeFiber(runtime, fiberId, now, {
    updateDetachedActorStatus: (id, status) => updateDetachedActorStatus(runtime, id, status),
    publishSchedulerSignal: (vm, updatedAt) => publishSchedulerSignal(runtime, vm, updatedAt),
  });
}
...
function findLastAssistantText(messages: readonly any[]): string {
...
function findLastToolText(messages: readonly any[]): string {
  for (let i = messages.length - 1; i >= 0; i--) {
    const msg = messages[i];
...
function isCooperativeAsyncWaitState(execState: any): boolean {
...
function hasInflightAsync(runtime: AiAgentOrchestratorRuntime): boolean {
  for (const [fiberId, ctx] of Object.entries(runtime.fiberIndex.snapshot())) {
    ...
    if (rec.status === "suspended" && isAsyncWaitReason(rec.waitingReason)) return true;
    if (rec.status === "ready" && isCooperativeAsyncWaitState(ctx.execState)) return true;
  }
  return false;
}
...
function resumePendingWakeMailboxFibersWhere(
  ...
  for (const [fiberId, ctx] of Object.entries(runtime.fiberIndex.snapshot())) {
    ...
    if (isTerminalFiberRecordStatus(rec.status) && rec.status !== "failed") continue;
    applyResumeFiberWithSignal(runtime, fiberId, now);
    resumed = true;
  }
  return resumed;
}
...
export function createAiAgentOrchestratorDriver(params: {
  ...
  const emitFiberSignal = (input: EmitFiberSignalParams): DurableControlSignalData | null => {
    ...
    if (emitted.signal.signalClass === "interrupt" || emitted.signal.signalClass === "wake") {
      applyResumeFiberWithSignal(runtime, input.fiberId, now);
      actorRuntime.sendFrom("control_signal", orchestratorId, "tick", { now, scope: "all" });
    } else {
  ...
  runtime = {
    ...
    state:
      ...
      createInitialOrchestratorState({
        agingStep: params.options?.agingStep,
        defaultSuspendPolicy: params.options?.defaultSuspendPolicy,
      }),
  ...
  return {
    ...
    settleInterruptedFiber: ({ fiberId, now, reason, controlKinds }) => {
      const ctx = runtime.fiberIndex.get(fiberId);
      if (!ctx) return;
      abortActorInflightWork(ctx.actor, ctx);
      resetCooperativeExecStateAfterInterrupt(ctx.execState);
      drainControlKinds(ctx.actor, new Set(controlKinds ?? ["cancel_requested", "shutdown_requested"]));
      runtime.pendingResumes.delete(fiberId);

      const current = runtime.state.fibers[fiberId];
      if (current && !isTerminalFiberRecordStatus(current.status)) {
        runtime.state = reduceOrchestrator(runtime.state, {
          type: "suspend",
          fiberId,
          now,
          reason: reason ?? ("idle_external" as FiberWaitingReason),
    ...
    tickUntilBlocked: async ({ now, maxTicks, maxWallMs }) => {
      ...
      while (tickCount < max) {
        ...
        if (!next) {
          ...
          // to enqueue asyncCompletion + resume_fiber before returning.
          if (
            hasInflightAsync(runtime)
            || runtime.pendingResumes.size > 0
            || hasRunningFibers(runtime.state)
            || hasPendingWakeMailboxWhere(runtime, () => true)
          ) {
            if (resumePendingWakeMailboxFibersWhere(runtime, () => true, now)) {
              continue;
            }
      ...
      while (tickCount < max) {
        ...
        if (!nextBackground) {
          ...
          if (
            ...
            hasPendingWakeMailboxWhere(runtime, isBackground) ||
            runtime.backgroundTasks.size > 0
          ) {
            if (resumePendingWakeMailboxFibersWhere(runtime, isBackground, now)) {
              continue;
            }
            await waitForNoRunningFibersWhere({
              getState: () => runtime.state,
        ...
        while (tickCount < max && !settled) {
          ...
          if (!next) {
            ...
            if (
              ...
              if (resumePendingWakeMailboxFibersWhere(runtime, () => true, Date.now())) {
                continue;
              }
              await waitForNoRunningFibers({
                getState: () => runtime.state,
