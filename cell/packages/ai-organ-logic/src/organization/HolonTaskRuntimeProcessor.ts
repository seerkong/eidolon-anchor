...
import type {
  ...
  HolonTaskRuntimeActorDispatchPort,
  HolonTaskRuntimeClockPort,
  HolonTaskRuntimeOrigin,
  HolonTaskRuntimeTimerPort,
} from "@cell/ai-organ-contract"
import type {
  ClosedValue,
  HolonExecutionInvocation,
...
export interface HolonTaskAssignmentPort {
  assign(
    input: Readonly<{
...
export interface HolonTaskMemberRuntimePort {
...
export interface HolonTaskProfilePort {
  normalize(value: unknown): HolonTaskExecutionProfile | Promise<HolonTaskExecutionProfile>
}

export interface HolonTaskProcessorRuntime {
  readonly taskManager: TaskManagerRuntime
  readonly assignment: HolonTaskAssignmentPort
  readonly memberRuntime: HolonTaskMemberRuntimePort
  readonly profile: HolonTaskProfilePort
  readonly actorDispatch: HolonTaskRuntimeActorDispatchPort
  readonly journal: Pick<HolonTaskPumpJournalPort, "dispatch">
  readonly clock: HolonTaskRuntimeClockPort
  readonly timer: HolonTaskRuntimeTimerPort
}

export interface ExecuteHolonTaskInput {
  readonly deploymentId: string
  readonly bindingRef: `resource://${string}`
  readonly holonRef: string
  readonly taskSpaceId: string
...
export interface HolonTaskExecutionResult {
  readonly deploymentId: string
  readonly taskSpaceId: string
  readonly taskId: string
  readonly memberRef: string
...
export class HolonTaskRuntimeProcessorError extends Error {
  constructor(readonly code: string, message: string) {
    super(`${code}: ${message}`)
    this.name = "HolonTaskRuntimeProcessorError"
  }
}

const invalid = (code: string, message: string): never => {
  throw new HolonTaskRuntimeProcessorError(code, message)
}

/**
 * Canonical TaskSpace execution Processor. Workflow is intentionally absent:
 * every external effect is supplied by the runtime and every invocation fact
 * is carried by the input.
 */
export async function executeHolonTask(
  runtime: HolonTaskProcessorRuntime,
  inputValue: ExecuteHolonTaskInput,
  config: TaskProcessorConfig,
): Promise<HolonTaskExecutionResult> {
  ...
  const assignment = await runtime.assignment.assign({
  ...
  const member = await runtime.memberRuntime.ensure({
    deploymentId: input.deploymentId,
    bindingRef: input.bindingRef,
    holonRef: input.holonRef,
  ...
  if (member.runtimeRef !== assignment.memberRuntimeRef || member.memberRef !== assignment.memberRef) {
    return invalid("EIDOLON_HOLON_MEMBER_RUNTIME_MISMATCH", "MemberRuntime output differs from the assignment.")
  }

  const existingStart = await runtime.taskManager.owner.readReceiptByCommand(
    input.taskSpaceId,
  ...
  if (!task || task.definition.kind !== "task") {
    return invalid("EIDOLON_HOLON_TASK_PROFILE_INVALID", "Started task lost its executable definition.")
  }
  const profile = await runtime.profile.normalize(task.profile)
  const definition = profile.facts.admission.definition
  if (definition.executionBinding.ref !== input.bindingRef || definition.rootHolonRef !== input.holonRef) {
    return invalid("EIDOLON_HOLON_TASK_TARGET_MISMATCH", "Started task no longer matches the frozen runtime definition.")
  }
  const invocation: HolonExecutionInvocation = Object.freeze({
    ...
    input: input.input,
    materialRefs: definition.output.materialPortRefs,
    resultContractRef: definition.output.schemaRef,
  })
  ...
  const dispatchStartedAt = runtime.clock.nowEpochMs()
  ...
  let dispatched: Awaited<ReturnType<HolonTaskProcessorRuntime["journal"]["dispatch"]>>
  try {
    dispatched = await runtime.journal.dispatch(intent, async (idempotencyKey) => {
      try {
        return (await runtime.actorDispatch.dispatch({
        ...
        throw new HolonTaskRuntimeProcessorError(
  ...
  const elapsedSettlementAt = new Date(
    Date.parse(startedAt) + (runtime.clock.nowEpochMs() - dispatchStartedAt),
  ).toISOString()
  const settledAt = laterTimestamp(
    input.settledAt,
    laterTimestamp(elapsedSettlementAt, settlementClaim.heartbeatAt, 1),
    1,
  )
  ...
  const materials = settlementMaterials(definition.output.materialPortRefs, dispatched.receipt.output)
...
async function maintainClaimHeartbeat(
  runtime: HolonTaskProcessorRuntime,
  input: ExecuteHolonTaskInput,
  initialClaim: TaskClaimToken,
  config: TaskProcessorConfig,
  signal: AbortSignal,
): Promise<void> {
  const intervalMs = Math.max(1, Math.floor(input.leaseDurationMs / 3))
  const startedAt = runtime.clock.nowEpochMs()
  const authorityStart = Date.parse(initialClaim.heartbeatAt)
  let claim = initialClaim
  while (!signal.aborted) {
    if (!await runtime.timer.wait(intervalMs, signal)) return
    const heartbeatAt = new Date(authorityStart + (runtime.clock.nowEpochMs() - startedAt)).toISOString()
...
async function replayResult(
  ...
  const member = await runtime.memberRuntime.resolve({
    deploymentId: input.deploymentId,
    runtimeRef: (assignmentValue as TaskClaimReceipt).claim.assigneeRef,
  })
  if (!member) {
...
function assertRuntime(runtime: HolonTaskProcessorRuntime): void {
  if (!runtime || typeof runtime !== "object"
    || typeof runtime.taskManager?.owner?.readSnapshot !== "function"
    || typeof runtime.assignment?.assign !== "function"
    || typeof runtime.memberRuntime?.ensure !== "function"
    || typeof runtime.memberRuntime?.resolve !== "function"
    || typeof runtime.profile?.normalize !== "function"
    || typeof runtime.actorDispatch?.dispatch !== "function"
    || typeof runtime.journal?.dispatch !== "function"
    || typeof runtime.clock?.nowEpochMs !== "function"
    || typeof runtime.timer?.wait !== "function") {
    invalid("EIDOLON_HOLON_TASK_RUNTIME_INVALID", "Task execution runtime ports are incomplete.")
  }
}

function normalizeInput(value: unknown): ExecuteHolonTaskInput {
  const required = [
    "deploymentId", "bindingRef", "holonRef", "taskSpaceId", "taskId", "origin",
    "assignmentCommandId", "startCommandId", "settlementCommandId", "invocationRef",
  ...
  if (!bindingRef.startsWith("resource://") || !bindingRef.slice("resource://".length)
    || bindingRef.slice("resource://".length).includes("://")) {
  ...
  return Object.freeze({
    ...
    taskId: text("taskId"),
    origin: normalizeHolonTaskRuntimeOrigin(fields.origin),
