...
function renderFlowCode(command: {
  ...
  return [
    ...
    ``,
    `type AuthoredAgentRuntime = { readonly ai: AIWorkflowStepExtensionAuthoredRuntimeContext }`,
    ``,
    ...
    `}`,
    ``,
    `export function writeMaterial(runtime: any, input: unknown, config: Record<string, unknown> = {}) {`,
    `  const run = runtime.ai.metadata.run`,
    `  const nodeId = String(config.nodeId ?? config.node_id ?? "material-write")`,
    `  return runtime.ai.effects.invoke({`,
    `    effectId: String(config.effectId ?? \`${'${run.runId}'}:${'${run.generation}'}:${'${nodeId}'}\`),`,
