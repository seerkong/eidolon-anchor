...
import { tickAiAgentRuntimeBackground } from "@cell/ai-organ-logic/runtime/tickAiAgentRuntimeBackground";
...
describe("AiAgentOrchestratorDriver.tickUntilBackgroundSettled", () => {
  ...
  it("runtime background pump does not advance ready interactive fibers", async () => {
    const main = createActor({ key: "main" });
    const detached = createActor({ key: "detached" });
    const vm = createVM({ controlActorKey: "main", actors: { main, detached } });
    ...
    const driver = createAiAgentOrchestratorDriver({
      fibers: [
        { fiberId: mainFiberId, vm, actor: main, messages: [], basePriority: 100, lane: "interactive" },
        { fiberId: detachedFiberId, vm, actor: detached, messages: [], basePriority: 1, lane: "detached" },
      ],
      runStep: async (ctx) => {
    ...
    await tickAiAgentRuntimeBackground({ vm, driver, now: Date.now(), maxWallMs: 2000 });

    const state = driver.getState();
    expect(mainSteps).toBe(0);
    expect(detachedSteps).toBeGreaterThan(0);
    expect(state.fibers[mainFiberId].status).toBe("ready");
    expect(state.fibers[detachedFiberId].status).toBe("completed");
  });
