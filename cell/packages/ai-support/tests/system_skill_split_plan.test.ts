...
describe("Eidolon Anchor system Skill split plan", () => {
  ...
  test("copies every published Flow DSL byte and provenance fact into Authoring", async () => {
    ...
    expect(provenance.packageVersion).toBe("0.2.0")
  ...
  test("keeps sibling delegation exact and initial coding context progressive", () => {
    ...
    expect(agentDefinition).toContain("{ byName: \"requirements-reviewer\" }")
    expect(agentDefinition).toContain("{ byId: previous.instance.instanceId }")
    ...
    expect(agentExecution).toContain("byName")
    expect(agentExecution).toContain("byId")
