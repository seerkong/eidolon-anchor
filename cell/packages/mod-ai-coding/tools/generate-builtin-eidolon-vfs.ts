...
function promptResource(id: string, description: string, content: string): string {
  if (content.includes("</?>")) throw new TypeError(`Prompt '${id}' contains the reserved XNL text terminator`)
  return `<Prompt #${id} apiVersion="depa.flows/v1" version="1.0.0" { lifecycle = "Active" description = "${description}" } (\n  <Content ?>${content.trim()}</?>\n)>\n`
}

function kindDefinition(kind: string): string {
  return `<KindDefinition #eidolon.coding.kind.${kind} apiVersion="halfcode.resources/v1" version="1.0.0" {\n  lifecycle = "Stable" resourceKind = "${kind}" sourceShapes = ["single-file"]\n  currentApiVersion = "depa.flows/v1" supportedApiVersions = ["depa.flows/v1"]\n}>\n`
}
...
export function builtinEidolonResourceFiles(): Readonly<Record<string, string>> {
  ...
  const files: Record<string, string> = {
    ...
    "MessageSources/WorkspaceAgents.xnl": `<AgentMessageSource #eidolon.coding.WorkspaceAgents apiVersion="depa.flows/v1" version="1.0.0" { lifecycle = "Active" description = "Workspace AGENTS.md instruction source" } (\n  <Content ?>${JSON.stringify({ implementation: "eidolon.workspace-agents/v1" })}</?>\n)>\n`,
    "ContextPipelines/StandardContext.xnl": `<AgentContextPipeline #eidolon.coding.StandardContext apiVersion="depa.flows/v1" version="1.0.0" { lifecycle = "Active" description = "Canonical Eidolon history and provider context pipeline" } (\n  <Content ?>${JSON.stringify(CONTEXT_PIPELINE)}</?>\n)>\n`,
  }
  for (const kind of ["AIAgentDefinition", "Prompt", "AgentMessageSource", "AgentContextPipeline", "Tool"]) {
    files[`KindDefinitions/${kind}/manifest.xnl`] = kindDefinition(kind)
  }
  for (const name of TOOL_NAMES) {
    files[`Tools/${name}.xnl`] = `<Tool #${name} apiVersion="depa.flows/v1" version="1.0.0" { lifecycle = "Stable" description = "Eidolon ${name} execution capability" }>\n`
  }
