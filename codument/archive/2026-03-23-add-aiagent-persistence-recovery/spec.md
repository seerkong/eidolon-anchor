...
#### Scenario: team protocol autonomy 查询与恢复路径保持 session-scoped
- **GIVEN** 两个不同 session 在同一进程中并存，且各自拥有 teammate、protocol records 与 autonomy workers
- **WHEN** 用户查询 `/team list`、`/protocol status`、`/autonomy status` 或系统恢复其中一个 session
- **THEN** 系统 MUST 只读取并暴露当前 session 的 runtime state
- **AND** 系统 MUST NOT 合并或泄漏来自其他 session 的 process-global team、protocol 或 autonomy 状态
...
- **GIVEN** teammate、protocol、questionnaire 或消息可见性依赖 actor 当前状态
...
- **GIVEN** actor 具有身份、systemPrompts、tool policy、protocol state、task tree、pending questionnaires 与 all mailboxes 等 durable data
...
- **GIVEN** actor 拥有 `control`、`childDone`、`teamInbox`、`humanInput`、`toolResult`、`aiGenerated` 等 mailboxes
- **WHEN** 系统写入 actor snapshot
...
### Requirement: Protocol and autonomy state derive from actor/vm snapshot
系统 MUST 让 protocol records 与 autonomy metadata 的恢复建立在 actor snapshot、fiber snapshot 与 VM durable subset 之上。

#### Scenario: 恢复 shutdown / plan approval 记录
- **GIVEN** actor 在退出前持有 shutdown 或 plan approval 的协议状态
- **WHEN** session 被恢复
- **THEN** 系统 MUST 恢复 request_id、状态与关键决策信息
- **AND** 协议状态查询工具可返回恢复后的结果
...
#### Scenario: protocol 查询与恢复在多 session 并存时保持隔离
- **GIVEN** 两个不同 session 在同一进程中各自拥有 protocol records
- **WHEN** 用户在其中一个 session 查询 `/protocol status` 或 `/shutdown-status`，或系统恢复其中一个 session
- **THEN** 系统 MUST 只读取当前 session 的 protocol state
...
#### Scenario: 恢复后 team/bg/protocol/autonomy 状态可查询
- **GIVEN** session 已恢复
- **WHEN** 用户使用 `/team list`、`/team status`、`/bg list`、`/bg status`、`/protocol status`、`/autonomy status`
...
- **WHEN** 系统恢复该 session
- **THEN** 系统 MUST 仍可从 VM snapshot、actor snapshot、fiber snapshot 与 actor transcript 重建 team/bg/protocol/autonomy 状态
...
- **WHEN** 用户再次通过 tools 或 TUI 查询 team、background、protocol 或 autonomy 状态
...
#### Scenario: autonomy claim 在主控制台可见
- **GIVEN** autonomy runner 成功将某个 pending task 派发给 autonomy teammate
- **WHEN** 该 claim 发生在当前 session 中
- **THEN** 主 agent / TUI MUST 能看到一条可读的 claim 提示，说明任务被分配给了哪个 autonomy teammate
- **AND** 该提示 SHOULD 带上任务摘要，避免用户只能去查底层 transcript 或 snapshot

#### Scenario: recovery report 通过 typed runtime output 暴露
- **GIVEN** session 在恢复过程中出现损坏文件、compat fallback 或 transcript 来源切换
- **WHEN** runtime 完成恢复
- **THEN** 系统 MUST 通过显式 typed runtime output 暴露 recovery report
- **AND** 调用方 MUST NOT 只能依赖 metadata 魔法字段读取这些恢复诊断信息

### Requirement: Orchestration history remains audit-only and uses session logs layout
系统 MUST 将 `orchestration_history.txt` 作为审计日志保存在 session 的 `logs/` 目录下，并保持其非恢复主源语义。

#### Scenario: orchestration history 写入 logs 目录
- **GIVEN** session 运行中产生 background、team、protocol 或 autonomy 审计事件
- **WHEN** 系统写入 orchestration history
- **THEN** 系统 MUST 将日志写入 `logs/orchestration_history.txt`
- **AND** 该日志 MUST NOT 成为 runtime 恢复主输入

#### Scenario: legacy 根目录 orchestration history 布局保持兼容迁移
- **GIVEN** 旧 session 仍在根目录持有 `orchestration_history.txt`
- **WHEN** 新实现继续写入或备份该 session 的 orchestration history
- **THEN** 系统 MUST 将其兼容迁移到 `logs/` 目录或按等价兼容方式处理
- **AND** 迁移过程 MUST NOT 导致已有审计日志静默丢失

### Requirement: Side-effect safety default
系统 MUST 在第一阶段将所有未完成外部副作用按 `no_replay` 语义处理。

#### Scenario: 恢复时不自动重放外部副作用
- **GIVEN** 某未完成任务可能已经执行了外部副作用
- **WHEN** session 被恢复
- **THEN** 系统 MUST NOT 自动重放该副作用
- **AND** 系统 MUST 将该任务呈现为需要人工判断、显式重提或保守解释的状态
