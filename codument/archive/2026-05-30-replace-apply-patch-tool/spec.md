...
Replace the existing project-local `ApplyPatch` implementation with a safer and more capable patch-first file editing tool. The new implementation must keep compatibility with this project’s current tool surface where needed, while migrating the related mechanisms that make apply-patch safe for coding agents: named anchors, multi-mode hunk matching, staged writes, permission/path alignment, freshness/context refresh integration, structured output, and prompt guidance.
...
- Existing ApplyPatch implementation is replaced or upgraded in-place, not left as an inconsistent parallel implementation.
- Tests cover parser behavior, named anchors, matching modes, ambiguous failures, add/update/delete/move guards, structured output, and permission/freshness integration where feasible.
- Successful output keeps unified diff compatibility and includes new metadata.
- Tool prompt/detail guidance documents patch-first usage and failure recovery.
- A final gap-loop checkpoint is configured for this track.

## Out of Scope

- Rewriting unrelated tools such as `Read`, `Edit`, `Write`, `Bash`, except for minimal integration needed by ApplyPatch freshness/context refresh or prompt guidance.
- Changing the broader actor orchestration model.
- Introducing new external dependencies unless implementation proves they are strictly necessary.
