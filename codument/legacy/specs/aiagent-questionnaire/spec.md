# AIAgent Questionnaire 能力规范

## 概述

本能力规范定义 AIAgent 中以 `Questionnaire` 工具替代旧 Confirm 机制后的当前真相，包括结构化问卷请求/回答事件、终端展示、回答解析以及等待策略。
...
- **THEN** 请求包含多个 `questions`
- **AND** 每个问题都有 `id`、`prompt`、`type`
...
系统 MUST 提供 `parseQuestionnaireAnswer()`，通过一次 LLM 调用，将 `QuestionnaireRequest` 与用户文本回答解析为 `QuestionnaireResult` 所需的结构化答案。
...
- **THEN** 系统产生新的 `QuestionnaireRequest(kind=clarification)`
- **AND** 原始文本与失败原因可被追踪
...
#### Scenario: 历史记录包含问卷请求与结果
- **GIVEN** 系统产生问卷请求与结果事件
- **WHEN** 写入消息历史
- **THEN** 历史中存在可回放的结构化记录
- **AND** 记录包含 request 与 result 的 payload
...
### Requirement: 等待策略可按请求配置
系统 MUST 支持 `pause_all` 与 `continue_others` 两种控制策略，并由每个 `QuestionnaireRequest` 通过 `suspendPolicy` 指定。
