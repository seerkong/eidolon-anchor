...
系统应当让 `@cell/ai-composer` 成为正式的 capability composition layer，用于解析 profile、组合 extension/bundle 并装配 runtime 所需能力。
...
- **THEN** `@cell/ai-composer` 应当负责解析这些能力并生成装配结果
...
- **WHEN** `@cell/ai-composer` 组装某个 runtime profile
...
系统应当让 `@cell/ai-composer` 输出稳定的 runtime assembly result contract，使 terminal 或其他 runtime 入口只消费装配结果，而不重新定义默认产品语义。
...
- **WHEN** 它消费 `@cell/ai-composer` 返回的 assembly result
- **THEN** 这些装配结果应作为 contract 的正式组成部分暴露
- **AND** `@cell/ai-composer` 不应重新退化成新的静态默认拼装中心
...
- **THEN** 这些 contract 类型应由 `@cell/ai-composer` 暴露
- **AND** extension 不应再通过 `@cell/mod-ai-kernel` 才能获得组合 contract

#### Scenario: Composer contract ownership does not create reverse package dependency cycles
- **GIVEN** `@cell/ai-composer` 已成为 extension-facing assembly contract 的正式宿主
- **WHEN** 默认 kernel/coding profile 需要聚合这些 mod 贡献形成内置 profile
- **THEN** 默认 profile 聚合层应位于 composer 之上的独立薄层，例如 `@cell/membrane`，而不是让 `@cell/ai-composer` 反向依赖默认 mod bundle
- **AND** `@cell/mod-ai-kernel` / `@cell/mod-ai-coding` 应继续只依赖 composer contract，而不与 composer 根包形成工作区依赖环

### Requirement: Default AI Domain Kernel Capability SHALL Move Into Mod AI Kernel
系统应当将默认 AI 领域 kernel baseline 收口到 `@cell/mod-ai-kernel`，而不是继续写死在 `core/organ/terminal` 中。

#### Scenario: Runtime needs default system capability
- **GIVEN** 某个 runtime 需要默认系统能力
- **WHEN** 它通过 composer/profile 进行装配
- **THEN** 这些能力应来自 `@cell/mod-ai-kernel`
...
- **THEN** 这些默认系统能力应由 `@cell/mod-ai-kernel` 暴露
...
- **THEN** 这些 baseline capability 应由 `@cell/mod-ai-kernel` 暴露
- **AND** `@cell/ai-composer` 与 `TerminalRuntime` 不应各自再维护一份平行默认定义

### Requirement: Default Coding Overlay SHALL Move Into Mod AI Coding
系统应当将默认 coding app 语义收口到 `@cell/mod-ai-coding`，包括 coding agent、coding prompt overlay、delegate-agent capability selection 与 coding policy。

#### Scenario: Runtime selects coding overlay
- **GIVEN** 某个 runtime 需要默认 coding app 语义
- **WHEN** 它通过 composer/profile 选择 coding overlay
- **THEN** 这些能力应来自 `@cell/mod-ai-coding`
...
- **THEN** `@cell/mod-ai-coding` 应以 overlay 或等价方式叠加这些差异
...
- **AND** 该差异不应再由 `TerminalRuntime` 或 `@cell/ai-composer` 私下补齐
