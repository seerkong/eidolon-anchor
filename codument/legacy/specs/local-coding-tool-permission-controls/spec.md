...
系统应当（SHALL）继续让 `bash`、`read`、`write`、`edit`、`multiedit`、`ls`、`glob`、`grep` 共享统一的本地权限求值语义，并允许 `exec` 兼容命令通过正式 profile 将 `ask` 转换为自动放行，而不是进入交互式审批等待。
...
#### Scenario: Full-auto auto-approves guarded in-workspace tool calls

- **GIVEN** 调用方以 `exec --full-auto -C <workspace>` 启动 headless exec
- **WHEN** 本地 coding 工具命中原本会进入 `ask` 的权限规则
- **THEN** 系统在 workspace 与显式额外可写目录边界内自动放行这些调用
- **AND** 不进入 questionnaire 阻塞链路

#### Scenario: Dangerous bypass disables approvals and workspace sandbox checks

- **GIVEN** 调用方以 `exec --dangerously-bypass-approvals-and-sandbox` 启动命令
- **WHEN** headless exec 运行并触发本地工具调用
- **THEN** 系统跳过审批等待
- **AND** 以危险模式运行，不再受 workspace-write 边界限制
- **AND** 该模式仍被明确标记为危险行为
