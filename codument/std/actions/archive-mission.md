...
归档一个已完成、取消、废弃或被替代的 mission。归档会把 mission 从 `pending/` 或 `active/` 移入 `archived/YYYY-MM-DD-<mission-id>/`，并按配置把完整 durable XNL decision tree 合并到长期 registry、提升 memory。
...
---- /?validate
---- #step ?track_precheck
扫描 mission.xml 中所有 `cdt:TrackLink state="bound"`：通过 id 解析 active track（codument/tracks/active/<id>/track.xml）或 archived track（codument/tracks/archived/**/<timestamp>-<id>/track.xml）。若存在 active 或 missing 的 bound track，issues-first 列出并请求用户选择：逐个调用 codument-archive-track 归档 eligible active tracks、保留不归档并继续、或停止归档。
---- /?track_precheck
---- #step ?promote_decisions
同时读取根 `decisions.xnl` 与递归 `decisions/**/*.xnl`，选择 eligible durable tree closure，按 stable id 在 staging 中合并到 `codument/decisions/**/*.xnl`；duplicate/conflict/validation failure 时不移动 mission。legacy Markdown 仅走显式兼容/迁移，不参与 canonical merge/index。
---- /?promote_decisions
---- #step ?promote_memory
...
- 不把 durable decision 投影为 canonical `decision.md`；archive summary 只是派生视图。
