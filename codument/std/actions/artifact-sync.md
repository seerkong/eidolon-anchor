...
> 本文是完整提示词（口径已对齐当前标准）。**程序化的执行流程**（resolve → generate → 按 policy 写目标，带 dry-run / conflict / provenance 分支）用流程标记块（` ```text ` + `@delimiter: --`，构造词汇见 [`_action-spec.md`](./_action-spec.md)）表达；**说明、规则、背景、示例**用 Markdown，内嵌 XML 用 ```` ```xml ```` 围栏。
>
...
---- /?user-id
---- #else-if ?hook cond="当前来自 action-hooks.xml 的 archive-track:after <cdt:ArtifactSync use=\"...\">"
