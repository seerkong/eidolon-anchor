# 校验 / 纠偏 / 确认协议（std/protocols/validation.md）

> 从原 `std/protocols.md` 拆出的「门控与纠偏」域。这是 `track.xml` / `action-hooks.xml` 里 `cdt:` hook 的**执行协议**（节点格式见 `std/spec/track-xml-spec.md §6`）。
...
- **归属**：若上层（action-hooks 或上层节点 hook）已拥有某 scope 的纠偏，下层不要再起竞争性 check，读已有结果即可。
