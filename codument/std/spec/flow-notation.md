...
- 有明确控制语义的规则：硬闸、一次一操作、等待回执、重试、漂移再收敛、收口。
...
| `#sequence` | 固定顺序执行的一组操作 |
| `#parallel` | 并行执行一组操作；可带 `limit` |
| `#step` | 单个操作或判断点 |
| `#if` / `#else-if` / `#else` | 局部分支 |
| `#loop` | 反馈循环、重试循环、reconcile loop |
| `#return` | 返回当前流程结果 |
| `#exit` | 发出操作后终止本回合，等待用户或工具回执 |
...
- `SKILL.md` 写全局事件路由：先判硬闸，再按事件读取精确 paths，再发唯一真操作或等待。
