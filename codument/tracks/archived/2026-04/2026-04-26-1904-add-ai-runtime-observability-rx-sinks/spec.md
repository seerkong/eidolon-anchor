# 规范：add-ai-runtime-observability-rx-sinks

## 概述

本 Track 为 AI runtime 增加响应式可观测能力。系统应在 VM private/public RxData 中增加专属 observability stream：所有可观测事实写入 private rx data，所有 sinks/consumers 通过 public rx data 只读消费。可观测失败不得影响用户协议输出。

本 Track 必须基于前三个已归档 Track 的实际设计：
- `AiAgentVm`、runtime ctx、snapshot、registry data 与 rx seam 已迁入 `ai-core-contract`，具体逻辑在 `ai-core-logic` / organ 层。
- `ensureVmRxData(vm)` 已提供 public readonly 面：`semanticEvents`、domain streams、`usage`、`traceSummary`。
- `createSemanticProtocolBinding(vm)` 已从 public RxData 派生 shared `SemanticProtocolFrame`，它属于用户协议投影层，不是可观测事实写入主路径。

因此本 Track 需要扩展 VM RxData 主 shape：新增标准化 observability stream。semantic events 可以被桥接为 observability records，但 provider capture、extension facts、runtime lifecycle 等非 semantic 来源也必须能够通过受控 helper 写入同一标准 observability stream。usage、traceSummary 等 signals 继续留在常规 public/private RxData 中，不作为 observability 专属 signal 重复建模。
...
## 验收标准
- `ObservabilityRxData`、sink binding 与 dispose 有测试覆盖。
- VM private/public RxData 已新增专属 observability stream，private 可写、public 只读。
- Observability projection/sinks 通过 public observability stream 消费，不直接订阅 eventBus 或读取 privateRxData。
- log sink 与 session trace artifact sink 至少一个完整场景通过。
- extension fact envelope/helper 有测试覆盖。
- sink 失败隔离有测试覆盖。

## 范围外事项
- 不要求实现完整 Phoenix exporter。
- 不实现跨 session trace 查询 UI。
- 不实现 evaluation/benchmark trace contract。
