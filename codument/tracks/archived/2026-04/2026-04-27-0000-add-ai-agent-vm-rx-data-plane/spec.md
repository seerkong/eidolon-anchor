# 规范：add-ai-agent-vm-rx-data-plane

## 概述

本 Track 在 `AiAgentVm` 上建立 public/private 响应式数据面。设计吸收参考项目 public/private rx data 的思想，但底层采用本项目已有 `vendor/depa-data-graph`，并尊重双层微内核边界。

本 Track 必须基于已完成的 `refactor-ai-agent-vm-runtime-shape`：`AiAgentVm`、runtime ctx、snapshot、registry data 与 rx seam 已迁入 `ai-core-contract`；`ai-core-logic` 负责 `createVM`、ensure helpers 和具体实现逻辑。因此本 Track 的首要任务是把 contract 中 `AiAgentVmPublicRxData` / `AiAgentVmPrivateRxData` 的 `null` 占位替换为稳定契约，并在 logic 包实现 depa-data-graph backed 数据面。
...
## 验收标准
- 新增 TypeScript 类型：private/public rx data 与 binding。
- `AiAgentVm` 中 `privateRxData`、`publicRxData` 的 contract 占位类型已被替换为真实协议类型。
- `ai-core-logic` 提供 `ensureVmRxData` 或等价 runtime op，并填充 `privateRxData`、`publicRxData`、`publicRxBinding`、`privateRxBinding`。
- semantic event stream、conversation domain streams 与 usage signal 至少有最小测试覆盖。
- sink/adapter 只能拿到 public 读面，不能直接写 private stream/signal。

## 范围外事项
- 不实现具体协议 adapter 输出。
- 不实现日志、Phoenix、trace artifact sinks。
