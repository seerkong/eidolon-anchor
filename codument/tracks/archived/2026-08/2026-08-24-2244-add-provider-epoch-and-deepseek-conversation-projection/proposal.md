...
- 把 DeepSeek official 与显式兼容 endpoint 的差异放在 provider compatibility profile 中，不按模型名或 URL 猜测。
...
- 新增 closed Chat wire projection 和 provider compatibility profile。
