...
### 3. Adapter-owned Wire Protocol Profiles（2026-08-31 Replan）

- wire profile 表达协议族，而不是供应商身份：`openai-responses@1`、`openai-chat@1`、`deepseek-chat@1`、`anthropic-chat@1`。
- DeepSeek adapter/driver 固定声明 `deepseek-chat@1`；官方 endpoint、SiliconFlow、iqingwa 或其他兼容网关均使用同一 closed request/reasoning/tool contract。
- provider id、base URL、model id 和 credentials 只参与连接与路由，不得选择 profile，也不允许 `compatibility_profile` 配置覆盖 adapter 的协议声明。
- 旧持久化 receipt 中的 `deepseek-official-chat@1` 和 `deepseek-compatible-chat@1` 在 authority 边界读取时归一化为 `deepseek-chat@1`；新 receipt、观测和报告只写 canonical id。
- `model_capabilities`、`cache_profile` 仍是 runtime observation，不进入 wire body；所有 DeepSeek 网关共享 cache unit、cost normalization 和 SLO 计算，不再按“官方/兼容”分组排斥。
...
- gate 前后记录原件文件树的稳定相对路径、size、mtime与SHA-256清单并要求完全相等；副本使用当前编译产物执行，DeepSeek adapter 自行声明 canonical `deepseek-chat@1`，禁止现场或 CLI 覆盖 profile。
...
- endpoint 能力若确有协议差异，应由新的 adapter/protocol profile 表达，而不是在同一 DeepSeek adapter 内按 provider 名称分支。
...
- protocol profile由adapter声明；所有DeepSeek网关统一为`deepseek-chat@1`。
