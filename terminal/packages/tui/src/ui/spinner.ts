...
function calculateColorIndex(
  ...
  charIndex: number,
  totalChars: number,
  options: Pick<AdvancedGradientOptions, "direction" | "holdFrames" | "trailLength" | "mirror">,
  state?: ScannerState,
): number {
  ...
  const visualIndex = options.mirror ? totalChars - 1 - charIndex : charIndex

  // Calculate directional distance (positive means trailing behind)
  const directionalDistance = isMovingForward
    ? activePosition - visualIndex // For forward: trail is to the left (lower indices)
    : visualIndex - activePosition // For backward: trail is to the right (higher indices)
